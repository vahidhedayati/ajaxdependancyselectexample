<g:javascript>
	...
	function ${attrs.id}Select(value) {
		//alert ('Value '+value);
		var updateDiv = document.getElementById('${attrs.setId}');
		var showDiv = document.getElementById('${attrs.showId}');
		//$('#${attrs.setId}').attr('bindValue',value);
		if (updateDiv) {
			//$('#${attrs.setId}').attr('bindValue',value);
			
			//setInterval( function() { $("#refreshable").load(location.href+" #${attrs.setId}*", "");}, 5000);
			
		//	 alert(' 00000000 before altering => ' + $('#${attrs.setId}').attr('bindValue'));
   // 
  //  alert(' after altering => ' + $('#${attrs.setId}').attr('bindValue'));
			//$('#'+showDiv).show();
			//$("#"+showDiv).toggle();
			$('#${attrs.showId}').css("display", "block");
			$('#${attrs.showId}').attr('value', value);
			//$('#${attrs.setId}').attr('bindValue',value);
		}
	}
